// ---------------------------------------------------------------------------
// TestLink TestStep Renderer -- a class to render recorded tests to a Test Link
// test format.
// ---------------------------------------------------------------------------
...
CasperRenderer.prototype.writeStep = function(stepDescription){

this.document.writeln("&lt;step&gt;");this.space();
this.document.writeln("&lt;step_number&gt;&lt;![CDATA[]]&gt;&lt;/step_number&gt;");this.space();
this.document.writeln("&lt;actions&gt;&lt;![CDATA[" + stepDescription + "]]&gt;&lt;/actions&gt;");
this.document.writeln("&lt;expectedresults&gt;&lt;![CDATA[]]&gt;&lt;/expectedresults&gt;");
this.document.writeln("&lt;execution_type&gt;&lt;![CDATA[1]]&gt;&lt;/execution_type&gt;");
this.document.writeln("&lt;/step&gt;");
}
...
CasperRenderer.prototype.writeHeader = function() {
  var date = new Date();
  this.text("/*==============================================================================*/", 0);
  this.text("/* Test Case generated " + date + " */", 0);
  this.space();
  this.text("/*Directly import-able to Test Link*/",0);
  this.text("/*==============================================================================*/", 0);
  this.space();
  this.space();
  this.text("&lt;?xml version='1.0' encoding='UTF-8'?&gt;",0); 
  this.space();
  this.text("&lt;testcases&gt;",0);this.space();
  this.text("&lt;testcase internalid = '' name='TESTCASE-" + date + "'&gt;",0);this.space();
  this.text("&lt;node_order&gt;&lt;/node_order&gt;",0);this.space();
  this.text("&lt;externalid&gt;&lt;/externalid&gt;",0);this.space();
  this.text("&lt;version&gt;&lt;/version&gt;",0);this.space();
  this.text("&lt;summary&gt;&lt;/summary&gt;",0);this.space();
  this.text("&lt;preconditions&gt;&lt;/preconditions&gt;",0);this.space();
  this.text("&lt;execution_type&gt;&lt;![CDATA[1]]&gt;&lt;/execution_type&gt;",0);this.space();
  this.text("&lt;importance&gt;&lt;/importance&gt;",0);this.space();
  this.text("&lt;estimated_exec_duration&gt;&lt;/estimated_exec_duration&gt;",0);this.space();
  this.text("&lt;status&gt;1&lt;/status&gt;",0);this.space();
  this.text("&lt;steps&gt;",0);
}
CasperRenderer.prototype.writeFooter = function() {
	this.space();
	this.text("&lt;/steps&gt;",0);this.space();
	this.text("&lt;/testcase&gt;",0);this.space();
	this.text("&lt;/testcases&gt;",0);
  }
...
CasperRenderer.prototype.startUrl = function(item) {
  ...
  this.text("******************************* Test Begin ********************************",0);
  
  this.text("Base Url:" + url,0);
...
CasperRenderer.prototype.openUrl = function(item) {
  ...
  if (url == history[history.length - 2]) {
    ...
    this.writeStep("Press browser back button");
...
CasperRenderer.prototype.click = function(item) {
  ...
  if(this.with_xy && !(tag == 'a' || tag == 'input' || tag == 'button')) {
    ...
    this.writeStep("Click on '" + item.info.innerText + "'.", 0);
  }
}
...
CasperRenderer.prototype.keypress = function(item) {
	...
	var test_object = item.info.innerText;
  
  	this.writeStep("Enter '" + text + "' in " + test_object + ".");
}
...
CasperRenderer.prototype.comment = function(item) {
  ...
  for (var i=0; i < lines.length; i++) {
    this.writeStep("COMMENT: '" + lines[i] + "'",0);
  }
}
...
CasperRenderer.prototype.checkText = function(item) {
  ...
  if ((item.info.type == "submit") || (item.info.type == "button")) {
      ...
      selector = this.pyrepr(item.text, true);
  } else {
      //selector = 'x("//*[normalize-space(text())='+this.cleanStringForXpath(item.text, true)+']")';
      selector = this.cleanStringForXpath(item.text, true);
  }
  this.waitAndTestSelector(selector);
}
...
CasperRenderer.prototype.checkHref = function(item) {
  ...
  if(xpath_selector) {
    ...
    this.writeStep("Validate if " + selector + " exists.");
}
