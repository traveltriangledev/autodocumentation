...
TestRecorder.ElementInfo = function(element) {
    ...
    this.innerText = this.findInnerText(element);
    
}

TestRecorder.ElementInfo.prototype.findInnerText = function(element){
	...
	for(var i = 0;i<n;i++){
		...
		map.set(atts[i].name, atts[i].value);
	...
	else if(map.has("value") && map.get("value") != ""){
		inner_text = map.get("value");
	}
	else if(map.has("placeholder") && map.get("placeholder") != ""){
		inner_text = map.get("placeholder");
	}
	else if(map.has("id") && map.get("id") != ""){
		inner_text = map.get("id"); 
	}
	else if(map.has("name") && map.get("name") != ""){
		inner_text = map.get("name"); 
	}
	
	//var setOfAttributes = new Set(arr);
	
	/*if(element.innerText != ""){
		inner_text = element.innerText;
	}
	else if((setOfAttributes.has("value"))){
		inner_text = element.value;
	}
	else if((setOfAttributes.has("placeholder"))){
		inner_text = element.placeholder;
		alert(element.placeholder);
	}
	else if((setOfAttributes.has("id"))){
		inner_text = element.id; 
	}
	else if((setOfAttributes.has("name"))){
